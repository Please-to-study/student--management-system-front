<template>
  <PageWrapper title="常箐数管理系统设置" contentFullHeight>
    <CollapseContainer title="基础设置">
      <BasicForm
        autoFocusFirstItem
        :labelWidth="200"
        :schemas="schemas"
        ...
        @reset="handleReset"
      >
        <template #payStyleButton>
          <a-button type="primary" class="my-4" @click="handlePayStyleCreate">
            课时费设置
          </a-button>
        </template>
    ...
    <PayStyleModal @register="payStyleRegister" />
  </PageWrapper>
</template>
<script lang="ts" setup>
  ...
  import PayStyleModal from './PayStyleModal.vue';

  const [payStyleRegister, { openModal: openPayStyleModal }] = useModal();

  function handlePayStyleCreate() {
    openPayStyleModal(true, {
      isUpdate: false,
    });
  }
  ...
  const schemas: FormSchema[] = [
    ...
    {
      field: 'payStyleButton',
      label: '',
      slot: 'payStyleButton',
    },
    {
      field: 'divider-CourseCategory',
      component: 'Divider',
      label: '课程类别设置',
      colProps: {
        span: 24,
