...
import { GetUserInfoModel, LoginParams, UserInfoParams } from '@/api/sys/model/userModel';
import { doLogout, getUserInfo, loginApi, Identity } from '@/api/sys/user';
  ...
  actions: {
    ...
    async login(
      ...
      try {
        ...
        // save token
        this.setToken(token);
        return this.afterLoginAction(userInfoParams, goHome);
      } catch (error) {
    ...
    async getUserInfoAction(userInfoParams: UserInfoParams): Promise<UserInfo | null> {
      if (!this.getToken) return null;
